...
import com.lianlian.global.payment.sdk.request.LinePayReq;
import com.lianlian.global.payment.sdk.response.LinePayResp;
 ...
 * @since 1.0
 */
public class LinePayTest extends BaseTest {

    @Test
    public void apply() {

        LinePayReq request = LinePayReq.builder().version("v1")
                .service(Service.LINE_PAY)
                .merchant_id(merchantId)
                .merchant_order_id("llgp-sdk-java-test-linepay-001")
                .order_amount("42.00")
                .order_currency(Currency.THB.name())
                .order_desc("linepay test")
                .payment_method("LINE_PAY_TH")
                .notify_url("https://www.yezhou.cc/callback/notify.php")
                .redirect_url("https://www.yezhou.cc/callback/redirect.php")
                .customer(Customer.builder().merchant_user_id("macz006").full_name("line user").build())
                .products(Arrays.asList(
                        Product.builder().name("line product").description("test product").unit_price("42.00")
        ...
        LLPayResult<LinePayResp> applyResult = payClient.execute(request);
