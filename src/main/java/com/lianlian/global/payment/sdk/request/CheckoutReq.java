...
@NoArgsConstructor
@AllArgsConstructor
public class CheckoutReq implements LLPayRequest<CheckoutResp> {
    ...
    /**
     * for bankcard installment
     */
    private String interest_absorb;
    ...
    public String validate() {
        ...
        if (!StringUtils.isEmpty(interest_absorb) && !StringUtils.equals(interest_absorb, "MERCHANT") && !StringUtils
                .equals(interest_absorb, "USER")) {
            return "parameter [interest_absorb] invalid";
        }
        boolean isValid = !StringUtils.isEmpty(payment_method) && (payment_method.length() > 64 || !payment_method.matches
                (RegexConst.PAYMENT_TYPE));
        if (isValid) {
